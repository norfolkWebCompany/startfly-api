<cfcomponent extends="taffy.core.resource" taffy:uri="/reporting/listing/rating" hint="some hint about this resource">
	<cffunction name="post" access="public" output="false">
		...
		<cfloop query="listings">
			...
			<cfloop index="s1" list="#listSort(structKeyList(dateStruct),'numeric')#">
				...
				SELECT 
				IFNULL(avg(reviews.rating), 0) AS total
				FROM reviews 
				...
				AND reviews.listingID = #listings.ID#
				</cfquery>
		...
		<cfset result['data']['series'] = series />
			...
			<cfset objTools.runtime('post', '/reporting/listing/rating', (getTickCount() - sTime) ) />
