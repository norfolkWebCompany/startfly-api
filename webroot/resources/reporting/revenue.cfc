<cfcomponent extends="taffy.core.resource" taffy:uri="/reporting/revenue" hint="some hint about this resource">
	<cffunction name="post" access="public" output="false">
		...
		<cfloop index="s1" list="#listSort(structKeyList(dateStruct),'numeric')#">
			...
			FROM paymentsIn 
			LEFT JOIN listing ON paymentsIn.listingID = listing.ID
			WHERE month(paymentsIn.created) = #numberFormat(dateStruct[s1]['month'],'00')#
			AND year(paymentsIn.created) = #dateStruct[s1]['year']# 
			<cfif arguments.partnerID neq ''>
		...
		<cfset result['data']['series'] = series />
			...
			<cfset objTools.runtime('post', '/reporting/revenue', (getTickCount() - sTime) ) />
