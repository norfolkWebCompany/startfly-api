<cfcomponent extends="taffy.core.resource" taffy:uri="/reporting/activity/bookings/value" hint="some hint about this resource">
	<cffunction name="post" access="public" output="false">
		...
		<cfloop index="s1" list="#listSort(structKeyList(dateStruct),'numeric')#">
			...
			FROM bookings 
			LEFT JOIN listing ON bookings.listingID = listing.ID
			WHERE month(bookings.created) = #numberFormat(dateStruct[s1]['month'],'00')#
			AND year(bookings.created) = #dateStruct[s1]['year']# 
			AND bookings.status IN (1,2,3,5)
			<cfif arguments.partnerID neq ''>
		...
		<cfset result['data']['series'] = series />
			...
			<cfset objTools.runtime('post', '/reporting/activity/bookings/value', (getTickCount() - sTime) ) />
