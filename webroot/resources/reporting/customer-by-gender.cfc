<cfcomponent extends="taffy.core.resource" taffy:uri="/reporting/customer/gender" hint="some hint about this resource">
	<cffunction name="post" access="public" output="false">
		...
		<cfloop query="listings">
			...
			<cfswitch expression="#listings.gender#">
				<cfcase value="M">
					<cfset labelName = 'Male: ' & listings.totalCount />
				</cfcase>
				<cfcase value="F">
					<cfset labelName = 'Female: ' & listings.totalCount />
				</cfcase>
				<cfdefaultcase>
		...
		<cfset result['data']['total'] = arraySum(listToArray(valueList(listings.totalCount))) />
			...
			<cfset objTools.runtime('post', '/reporting/customer/gender', (getTickCount() - sTime) ) />
